...
def route(task_type: str, prompt: str, tenant_tier: str, providers, usage_logger):
    ...
    for provider_name, model in chain:
        ...
        try:
            call = providers[provider_name]  # KeyError now caught below, not fatal
            ...
            usage_logger(task_type=task_type, model=model, provider=provider_name,
                         ...
                         **usage)
            return text
        except Exception as err:  # timeout, connection, missing provider, provider error -> next in chain
            last_err = err
            continue
