...
def _resolve_service_candidates(tenant_id: int, service_name_or_id) -> list[dict]:
    """Core matching logic, exposed so callers can tell the difference
    between 'no match' and 'genuine tie' — e.g. 'kids dance class'
    matching both the At Studio and At Home variants, or 'wedding
    choreography' matching both the single- and multi-performance rows.
    Returns [] (no match), [one match], or [2+ tied candidates].

    Full-word-coverage rule: a row only counts as a candidate if EVERY
    word in the query is found somewhere in that row's name. Partial
    overlap is NOT enough — found live 2026-07-26 that scoring on ANY
    shared word let "wedding dance" wrongly tie against every service
    with "dance" in its name (Kids Dance Class, Ladies Dance & Fitness),
    when it should match nothing at all."""
    q = get_supabase().table("services").select("*").eq("tenant_id", tenant_id).eq("is_active", True)
    if isinstance(service_name_or_id, int):
        row = (q.eq("id", service_name_or_id).execute().data or [None])[0]
    ...
    query_lower = service_name_or_id.lower().strip()

    # Tier 1: full-phrase containment, either direction — still the
    # strongest signal, handles spelling variants like 'hair colour' vs
    # 'Hair Coloring' cleanly.
    exact_matches = [
        row for row in rows
        if query_lower in row["name"].lower() or row["name"].lower() in query_lower
    ]
    if len(exact_matches) == 1:
        return exact_matches

    candidates = exact_matches if len(exact_matches) > 1 else rows

    query_words = set(_match_words(service_name_or_id))
    if not query_words:
        return []
    ...
    full_matches = [
        ...
        if all(
            any(qw in rw or rw in qw for rw in _match_words(row["name"]))
...
def _get_service(tenant_id: int, service_name_or_id) -> dict | None:
    """Thin wrapper kept for every existing ID-based caller (which always
    resolves to exactly one row or none — ties are only possible on
    free-text lookups). Returns None on a genuine tie, same fail-closed
    behavior as before this file was split."""
    candidates = _resolve_service_candidates(tenant_id, service_name_or_id)
...
async def create_booking(tenant: dict, service_name: str, start_time_iso: str,
    ...
    candidates = _resolve_service_candidates(tenant["id"], service_name)
    if len(candidates) > 1:
        names = "; ".join(c["name"] for c in candidates)
        return {"success": False, "result_text": f"A couple of options match '{service_name}': {names}. Which one would you like?"}
...
async def decide_and_run_booking_tool(text: str, tenant: dict, user_id: int | None,
    ...
    try:
        if name == "check_availability":
            sname_query = args.get("service_name", "")
            candidates = _resolve_service_candidates(tenant["id"], sname_query)
            if len(candidates) > 1:
                names = "; ".join(c["name"] for c in candidates)
                return {"result_text": f"A couple of options match '{sname_query}': {names}. Which one would you like?"}
            service = candidates[0] if candidates else None
            if not service:
                return {"result_text": f"We don't have a service matching '{sname_query}' — want me to list what we do offer?"}
        ...
        if name == "book_appointment":
            ...
            for sname in service_names:
                candidates = _resolve_service_candidates(tenant["id"], sname)
                if len(candidates) > 1:
                    names = "; ".join(c["name"] for c in candidates)
                    outcomes.append(f"A couple of options match '{sname}': {names}. Let me know which and I'll book the rest.")
                    continue
                svc = candidates[0] if candidates else None
                if not svc:
